...
from eval.chain import build_winner_take_all_weights, fetch_metagraph, get_validator_weight_target, parse_commitments, set_weights
...
from scripts.validator.config import EVAL_PROMPTS_FULL, EVAL_PROMPTS_H2H, MAX_KL_THRESHOLD, NETUID, REFERENCE_MODEL, REFERENCE_UID, TEACHER_MODEL
from scripts.validator.eval_orchestrator import add_top5_contenders, cap_challengers, check_models_exist, precheck_all_models, process_results, run_eval_on_pod, select_challengers
from scripts.validator.pod_manager import init_pod
from scripts.validator.side_effects import sync_king_runtime
from scripts.validator.state_manager import migrate_dq_entries, update_h2h_state, update_model_tracking, update_top4_leaderboard
...
def _log_git_revision():
    try:
        ...
        git_hash = subprocess.check_output(["git", "rev-parse", "--short=8", "HEAD"], cwd=repo_root, stderr=subprocess.DEVNULL).decode().strip()
        git_msg = subprocess.check_output(["git", "log", "--oneline", "-1"], cwd=repo_root, stderr=subprocess.DEVNULL).decode().strip()
...
def _sync_king_weights(subtensor, wallet, netuid, n_uids, king_uid, validator_uid, state_dir):
    ...
    if current_weight_target == king_uid:
        return
    logger.warning(f"Validator weights stale before eval: chain UID {current_weight_target} != king UID {king_uid}; syncing")
    log_event(f"Syncing stale weights before eval: chain UID {current_weight_target} -> king UID {king_uid}", level="warning", state_dir=state_dir)
    weights = build_winner_take_all_weights(n_uids, king_uid)
    set_weights(subtensor, wallet, netuid, n_uids, weights, king_uid)


def _persist_preliminary_results(results, models_to_eval, king_uid, state, current_block, current_block_hash, n_prompts, is_full_eval, king_kl):
    ...
    try:
        ...
        for model_name, student_result in results.get("students", {}).items():
            ...
            imm_h2h.append({"uid": model_uid, "model": model_name, "kl": round(model_kl, 6), "is_king": is_king, "vs_king": ""})
...
def _append_round_score_history(state, current_block, winner_uid, uid_to_hotkey):
    valid_scores = {uid_str: kl for uid_str, kl in state.scores.items() if uid_str not in state.dq_reasons and 0 < kl <= MAX_KL_THRESHOLD}
    if not valid_scores:
        return
    append_score_history(block=current_block, timestamp=time.time(), scores=valid_scores, king_uid=winner_uid, state_dir=state.state_dir, uid_to_hotkey=uid_to_hotkey)


def run_validator(network, netuid, wallet_name, hotkey_name, wallet_path, lium_api_key, lium_pod_name, state_dir, max_params_b, tempo, once, use_vllm):
    ...
    pod = init_pod(Lium(config=cfg), lium_pod_name, eval_script, "/home/pod_eval.py", TEACHER_MODEL)
    epoch_count = 0
    while True:
        try:
            ...
            if state.current_round and not state.eval_progress.get("active"):
                ...
                log_event("Cleared orphaned round state with no active eval progress" + (f" ({round_age_min:.1f}m old)" if round_age_min is not None else ""), level="warning", state_dir=state_dir)
                state.clear_round()
                state.current_round = {}
            ...
            try:
                ...
                log_event(f"Chain unreachable: {str(exc)[:150]}, retrying in 5min", level="error", state_dir=state_dir)
                time.sleep(300)
                continue
            ...
            valid_models, disqualified = precheck_all_models(commitments, uid_to_hotkey, uid_to_coldkey, state, max_params_b)
            ...
            validator_uid = next((uid for uid, hotkey in uid_to_hotkey.items() if hotkey == wallet.hotkey.ss58_address), None)
            ...
            if not challengers:
                ...
                if king_uid is not None:
                    set_weights(subtensor, wallet, netuid, n_uids, build_winner_take_all_weights(n_uids, king_uid), king_uid)
                state.save()
                if once:
                    break
            ...
            if REFERENCE_MODEL and REFERENCE_UID not in models_to_eval:
                models_to_eval[REFERENCE_UID] = {"model": REFERENCE_MODEL, "commit_block": 0, "hotkey": "reference", "is_reference": True}
            ...
            prompt_texts = [format_prompt(prompt) for prompt in epoch_prompts]
            state.current_round = {
            ...
            state.save_round()
            results = run_eval_on_pod(pod, models_to_eval, king_uid, n_prompts, prompt_texts, state, max_params_b, is_full_eval, use_vllm, eval_script, "/home/pod_eval.py")
            if results is None:
                logger.warning("Eval did not produce usable results — clearing round state")
                log_event("Eval failed to produce usable results; cleared round state and will retry next epoch", level="warning", state_dir=state_dir)
                state.clear_round()
                ...
                time.sleep(tempo)
                continue
            uid_to_model = _persist_preliminary_results(results, models_to_eval, king_uid, state, current_block, current_block_hash, n_prompts, is_full_eval, king_kl)
            winner_uid, winner_kl, h2h_results, king_h2h_kl, king_per_prompt, this_round_uids = process_results(results, models_to_eval, king_uid, state, uid_to_hotkey, commitments, n_prompts, current_block, king_kl, epoch_count, is_full_eval, epoch_start_time=epoch_start)
            if winner_uid is not None:
                set_weights(subtensor, wallet, netuid, n_uids, build_winner_take_all_weights(n_uids, winner_uid), winner_uid)
            else:
                logger.info("No valid miners — skipping weight setting")
            state.save()
            update_h2h_state(state, h2h_results, king_uid, winner_uid, king_h2h_kl, king_kl, king_per_prompt, current_block, n_prompts, is_full_eval, uid_to_model, valid_models, challengers, epoch_count, disqualified, block_hash=current_block_hash, epoch_start_time=epoch_start)
            ...
            sync_king_runtime(winner_uid != king_uid if king_uid is not None else False, effective_king_model, effective_king_uid)
            ...
            update_top4_leaderboard(state, winner_uid, king_uid, king_kl, h2h_results, uid_to_model, valid_models, current_block, epoch_count, disqualified)
            state.clear_round()
            ...
            if winner_uid is not None and winner_uid != king_uid and king_uid is not None:
                ...
                try:
                    announce_new_king(winner_uid, new_king_model, winner_kl, king_uid, old_king_model, old_kl, state, paired_prompts=winner_entry.get("paired_prompts") or winner_entry.get("prompts_scored"), total_prompts=winner_entry.get("prompts_total") or n_prompts, p_value=winner_tt.get("p"))
                except Exception as exc:
            ...
            if king_changed:
                log_event(f"Round complete. New king: UID {winner_uid} ({winner_model}), KL={winner_score:.6f}. Dethroned UID {king_uid}.", state_dir=state_dir)
            else:
                log_event(f"Round complete. Winner: UID {winner_uid}, KL={winner_score:.6f}. Weights set.", state_dir=state_dir)
            if once:
                break
